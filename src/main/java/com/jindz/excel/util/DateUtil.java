...
public class DateUtil {
	...
	public static Date strtodate1(String s) throws ParseException {
		try {
			Date date = new Date(new SimpleDateFormat(DATE_FORMATTER).parse(s).getTime());
			return date;
		} catch (ParseException e) {
			// e.printStackTrace();
			Date date1 = new Date(new SimpleDateFormat(DATE_FORMATTER_2).parse(s).getTime());
			return date1;
		}
	...
	public static java.sql.Date strtosqldate1(String s) {
		try {
			java.sql.Date date = new java.sql.Date(new SimpleDateFormat(DATE_FORMATTER).parse(s).getTime());
			return date;
		} catch (ParseException e) {
			e.printStackTrace();
