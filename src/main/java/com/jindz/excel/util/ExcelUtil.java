...
public abstract class ExcelUtil {
		...
		for (int i = 0; i < valueList.size(); i++) {
			...
			for (int j = 0; j < methodList.size(); j++) {
				...
				if (filed.isAnnotationPresent(Excel.class)) {
					...
					// set text
					setText(xssBook, sheet, i, toString(comment.get("title")), toInt(comment.get("index")), value, type.getValue(),
							toString(comment.get("timeFormat")), toString(comment.get("calendarFormat")));
					// set Style
					if (comment != null) {
	...
	private static Field getFieldByMethod(Method method, Object target) throws NoSuchFieldException, SecurityException {
		...
		throw new  NoSuchFieldException(method.getName());
	}
				...
				for (int i = 0; i < method.size(); i++) {
					try {
						...
						if (field.isAnnotationPresent(Excel.class)) {
							...
							validateTitleName(getFristRowTitleName(cell, xssfSheet.getRow(startLine - 1)), toString(parseComment(field).get("title")));
	...
	private static String getValue(XSSFCell xssfCell) {
		try {
			...
			if (format == 14 || format == 31 || format == 57 || format == 58 || format == 165 || format == 176) {
				...
				sdf = new SimpleDateFormat(FORMAT_FULL);
			} else {
			...
			try {
				...
				// TODO Auto-generated catch block
				// e.printStackTrace();
				// System.out.println("Cannot get a numeric value from a text cell:"
				// + xssfCell.toString());
				xssfCell.setCellType(Cell.CELL_TYPE_STRING);
				return String.valueOf(xssfCell.getStringCellValue());
			}
	...
	private static void setText(XSSFWorkbook xssBook, XSSFSheet sheet, int row, String title, int titleIndex, String value, Integer valueType,
			String timeFormat, String calendarFormat) throws Exception {
		if (row == 0) {
	...
	private static List<Method> getmethodList(String flag, Method[] method) {
		...
		for (int i = 0; i < method.length; i++) {
			if (method[i].getName().startsWith(flag)) {
